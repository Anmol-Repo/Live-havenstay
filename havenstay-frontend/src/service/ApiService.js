...
static ENCRYPTION_KEY = process.env.REACT_APP_ENCRYPTION_KEY;
 ...
 //decrypting token usign crypto.js
 ...
 static saveToken(token){
  ...
  //retreive token
    ...
    static async proceedForPayment(body) {
        ...
        (`${this.BASE_URL}/payments/pay`, body, {
            headers: this.getHeader()
        });
        return resp.data; //return the razorpay transaction id for this transaction
    }

    //TO UPDATE PAYMENT & VERIFY AFTER ITS COMPLETED
    static async verifyPayment(body) {
        const resp = await axios.post
        (`${this.BASE_URL}/payments/verify`, body, {
            headers: this.getHeader()
        });
        return resp.data;
    }
